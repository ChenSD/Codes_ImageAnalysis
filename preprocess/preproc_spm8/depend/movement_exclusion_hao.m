function movement_exclusion_hao(Config_File)
...
%-Configurations
ResPostFix     =paralist.YourName; %Lei Hao add 2017/03/10
...
if ~isempty(FullRunIndex)
  ...
  fid = fopen(['Log_MovementSummaryStats_',CondList{1,1},'_',ResPostFix,'.txt'], 'w+'); %Lei Hao add 2017/03/10
  fprintf(fid, '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n', 'TASK', 'Scan_ID', ...
    'Range x', 'Range y', 'Range z', 'Range pitch', 'Range roll', 'Range yaw', 'Max Displacement', ...
    'Max Scan-to-Scan Displacement', 'Mean Scan-to-Scan Displacement', 'Num Scans > 0.5 Voxel Displacement', ...
    'Num Scans > 5% Global Signal', '% of Volumes Repaired');
...
fprintf(['Please check: Log_MovementMissingInfo_',CondList{1,1},'_',ResPostFix,'.txt (if any) for subjects that do not have movement files\n']); %Lei Hao add 2017/03/10
%fprintf('Please check: MovementSummaryStats.txt for summary stats\n');
fprintf(['Please check: Log_MovementSummaryStats_',CondList{1,1},'_',ResPostFix,'.txt for summary stats\n']); %Lei Hao add 2017/03/10
disp('------------------------------------------------------------------');
end
